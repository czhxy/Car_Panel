# Car_Panel 汽车双 ECU 仪表盘项目方案（历史版 v2.1）

> 项目代号：**Car_Panel**
> 文档版本：v2.1（历史方案，已废弃）
> 目标：在双 MCU（STM32F407 + STM32F103）+ CAN 总线 架构上，模拟汽车"动力域 ECU + 显示域 ECU"，并实现 OTA/IAP、LVGL 仪表盘 UI、闭环电机控制。
> 状态：本方案曾规划 F407 + FSMC 8080 + W25Q64/AT24C02，当前仓库已迁移到 F429 + MSP2834 SPI 屏 + 内部 Flash 真 AB OTA。当前方案请参见 `docs/Car_Panel_F429_Project_Plan.md`。
> 说明：本文保留历史设计决策和废弃硬件资料，不应作为当前引脚、CAN 报文、OTA 流程或工程路径的依据。
...
| 14 | CAN 超时阈值 | 未给 | 历史草案曾设定显示域 100 ms 未收到 0x180、动力域 200 ms 未收到 0x200；当前协议已迁移为扩展 ID + mode_id，阈值仍待当前实现定义 | 给出可实现的具体值 |
| 15 | W25Q16 用途 | 笼统"固件暂存" | 历史方案规划板载 W25Q16 → 字库/图标，外置 W25Q64 → OTA 暂存 + 资源；当前 F429 方案不使用这些外置 Flash | 记录历史方案，不代表当前实现 |
...
> 资源清单来自随板原理图 `docs/schematic/STM32F407ZGT6开发板.PDF`，仅用于历史方案追溯。
...
**角色（历史方案）**：显示域 ECU 原规划为 Bootloader/IAP + LVGL 仪表盘；外置 W25Q64 用于 OTA 暂存 + 资源；AT24C02 用于升级标志。当前显示域已改为 STM32F429IGT6，采用内部 Flash 真 AB OTA。
...
**角色（历史方案）**：动力域 ECU 原规划为单电机闭环控制；当前工程已改为双 MG310 电机 + 双 DRV8833，并与 F429 通信。
...
### 6.1 STM32F407ZGT6 片上 Flash 分区（历史方案，当前废弃）
...
### 6.3 外置 W25Q64（8 MB）分区（历史方案，当前废弃）
...
### 6.4 板载 W25Q16（2 MB）分区（历史方案，当前废弃）
...
│   ├── schematic/                  # 原理图 PDF
│   └── notes/                      # 开发笔记
├── project/display_ecu_f429/       # 当前显示域工程；历史 F407 工程已废弃
│   ├── bootloader/
│   ├── app/
│   ├── common/
│   └── tools/
├── project/power_ecu_f103/         # 当前动力域工程（裸机）
└── tools/                          # 全局工具脚本
```
...
2. **建仓**：在 Car_Panel 目录初始化 git 仓库，分支策略：
   ...
   - `feature/*`：特性分支
   - `release/*`：发布前
3. **建工程**：历史上曾计划创建 `display_ecu_f407/bootloader`、`display_ecu_f407/app`、`power_ecu_f103` 三个 Keil 工程；当前工程路径为 `project/display_ecu_f429/` 和 `project/power_ecu_f103/`
4. **配置初始化代码**：历史上曾计划使用 STM32CubeMX；当前工程使用 STM32F4/F1 SPL 和 Keil 工程配置
5. **跑通 Phase 0**：硬件最小验证
...
- `*.ioc`（历史规划项；当前工程未使用 CubeMX 作为构建入口）
- 自定义 scatter 文件 `*.sct`
- `docs/Car_Panel_Project_Plan.md`（历史文档）
- `docs/Car_Panel_F429_Project_Plan.md`（当前方案文档）
...
> 当前方案和当前工程事实以 `docs/Car_Panel_F429_Project_Plan.md`、根目录 `AGENTS.md` 以及两个 ECU 的 `HANDOFF.md` 为准。
...
- TJA1050 数据手册
- W25Q64 / AT24C02 数据手册
- 板上原理图：`docs/schematic/STM32F407ZGT6开发板.PDF`
