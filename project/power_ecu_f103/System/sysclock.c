...
TaskPeriod_t * tp =NULL;
TaskPeriodFlag_t * tpf = NULL;
...
{	
	...
	now = sys_tick_now();
	if(now - tpf->task_period_1ms >= 1 * TICKS_PER_MS)
	{
		tp->period_1ms = now;
		tpf->task_period_1ms=1;
	}
		if(now - tpf->task_period_5ms >= 5 * TICKS_PER_MS)
	{
		tp->period_5ms = now;
		tpf->task_period_5ms=1;
	}
		if(now - tpf->task_period_10ms >= 10 * TICKS_PER_MS)
	{
		tp->period_10ms = now;
		tpf->task_period_10ms=1;
	}
		if(now - tpf->task_period_20ms >= 20 * TICKS_PER_MS)
	{
		tp->period_20ms = now;
		tpf->task_period_20ms=1;
	}
		if(now - tpf->task_period_100ms >= 100 * TICKS_PER_MS)
	{
		tp->period_100ms = now;
		tpf->task_period_100ms=1;
	}
		if(now - tpf->task_period_200ms >= 200 * TICKS_PER_MS)
	{
		tp->period_200ms = now;
		tpf->task_period_200ms=1;
	}
		if(now - tpf->task_period_500ms >= 500 * TICKS_PER_MS)
	{
		tp->period_500ms = now;
		tpf->task_period_500ms=1;
	}
		if(now - tpf->task_period_1000ms >= 1000 * TICKS_PER_MS)
	{
