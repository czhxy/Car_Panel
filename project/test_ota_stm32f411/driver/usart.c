...
void UART_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		UART_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void UART_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		UART_SendByte(Number / UART_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	UART_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
